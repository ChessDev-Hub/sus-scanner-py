...
class UserMetrics:
    ...
    wr_gap: float = 0.0  # tournament WR minus non-tournament WR
...
class SusScanner:
    """
    Suspicion scanner for Chess.com Daily games, with Tournament vs Non-Tournament comparison.
    """
    ...
    def analyze_usernames(self, usernames: List[str]) -> List[UserMetrics]:
        ...
        for u in usernames:
            try:
                ...
                # keep going; you can log this if you prefer
                print(f"[warn] {u}: {e}")
    ...
    def _fetch_daily_games(self, username: str, lookback_months: int) -> List[GameSummary]:
        ...
        # Build the set of year-month strings to include (current month excluded until complete, so go back N months)
        ym_to_include = set()
        today = dt.date.today()
